"""
Агент для приветствия клиентов
"""
from .base_agent import BaseAgent
from ..services.langgraph_service import LangGraphService
from .tools.call_manager_tools import CallManager


GREETING_STAGE_INSTRUCTION = """# РОЛЬ
Ты — AI-администратор салона красоты LookTown. 
Твой стиль общения — дружелюбный, но профессиональный и краткий, как у реального менеджера в мессенджере.
Всегда общайся на "вы" и от женского лица. 
Ты должна общаться как реальный человек, избегай роботизированных ответов, не вставляй в ответ таблицы.
Здоровайся с клиентом, если это его первое сообщение в чате, первое сообщение за день либо он с тобой поздоровался.


# ИНСТРУКЦИИ ДЛЯ СТАДИИ GREETING

Это начало диалога. Твоя задача — вежливо поздороваться и передать инициативу клиенту. Не предлагай услуги. Задай открытый, но направляющий вопрос. Инструменты на этой стадии не требуются.
Stage_code: 654

# ПРИМЕРЫ ОТВЕТОВ
Если здесь есть примеры которые тебе подходят, то используй в точности их.
Добрый день!
На связи менеджер LOOKTOWN 🌻

Чем я могу вам помочь?"""


class GreetingAgent(BaseAgent):
    """Агент для приветствия клиентов"""
    
    def __init__(self, langgraph_service: LangGraphService):
        super().__init__(
            langgraph_service=langgraph_service,
            instruction=GREETING_STAGE_INSTRUCTION,
            tools=[CallManager],
            agent_name="Агент приветствия"
        )


# Экспорт агента для совместимости с README
greeting_agent = None  # Будет инициализирован через LangGraphService


def get_greeting_agent(langgraph_service: LangGraphService):
    """Получить экземпляр агента приветствия"""
    return GreetingAgent(langgraph_service)

