...
Для понимания контекста обращай внимание на историю переписки: если идёт непрерывный процесс переписки, выбирай ту же стадию что и была раньше.
Но последнее сообщение должно быть ключевым:  если человек сначала узнавал про услуги, а потом сказал хочу записаться, то это уже стадия бронирования а не сбора информации. 
...
- booking: клиент хочет записаться на услугу (запишите, хочу на.., можно записаться? и т.д.), либо узнать когда есть доступное время для записи.
...
- tool_tester: Клиент просит протестировать инструменты, задает вопросы о работе инструментов, хочет проверить функциональность системы или инструментов ("протестируй инструмент", "как работает FindSlots?", "проверь GetCategories", "протестируй все инструменты").
